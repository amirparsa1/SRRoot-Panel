        ...
        if (request.method === "POST" && url.pathname === "/api/deploy") {
            try {
                ...
                if (subData.success && subData.result && subData.result.subdomain) {
                    ...
                    const newSub = `srroot-${Math.random().toString(36).substring(2, 8)}`;
                ...
                const workerName = `srroot-panel-${uniqueSuffix}`;
                const dbName = `srroot-db-${uniqueSuffix}`;
                ...
                formData.append("srroot.js", new Blob([srrootCode], { type: "application/javascript+module" }), "srroot.js");
        ...
        if (request.method === "POST" && url.pathname === "/api/list-panels") {
            try {
                ...
                for (let script of scriptsData.result) {
                    if (script.id.startsWith("srroot-panel") || script.id.startsWith("ez-")) {
...
<head>
    ...
    <title>SRRoot Panel Deployer</title>
    ...
    <style>
        ...
        .dark ::-webkit-scrollbar-thumb { background: #3d2b7a; }
        ::-webkit-scrollbar-thumb:hover { background: #9ca3af; }
        .dark ::-webkit-scrollbar-thumb:hover { background: #5b3fa0; }
        * { scrollbar-width: thin; scrollbar-color: #d1d5db transparent; }
        .dark * { scrollbar-color: #3d2b7a transparent; }
...
<body class="bg-gray-50 text-gray-900 dark:bg-sr-bg dark:text-zinc-100 min-h-screen flex flex-col items-center justify-center p-4">
    <div id="mainCard" class="w-full max-w-md bg-white dark:bg-sr-card border border-gray-200 dark:border-sr-border rounded-3xl shadow-2xl p-8 relative overflow-hidden z-10">
        <div class="absolute -left-12 -top-12 w-40 h-40 bg-cyan-500/10 rounded-full blur-3xl pointer-events-none"></div>
        <div class="absolute -right-12 -bottom-12 w-40 h-40 bg-violet-500/10 rounded-full blur-3xl pointer-events-none"></div>
            <div class="text-center mb-6 relative z-10">
                <div class="inline-flex items-center justify-center p-3 bg-violet-950/60 border border-violet-500 text-violet-400 rounded-2xl mb-4 shadow-[0_0_15px_rgba(139,92,246,0.4)] animate-glow">
            ...
            <h2 class="text-2xl font-black text-gray-900 dark:text-white mb-2">SRRoot Panel Deployer</h2>
            <p class="text-sm font-medium text-gray-500 dark:text-zinc-400">نصب خودکار پنل SRRoot روی کلودفلر</p>
            <p class="text-sm font-medium text-gray-500 dark:text-zinc-400 mt-1">🔥  روزانه ۱۰ الی ۱۰۰ گیگ کانفیگ رایگان  🔥</p>
        </div>
        <div class="space-y-5 relative z-10">
            <a href="https://dash.cloudflare.com/profile/api-tokens?permissionGroupKeys=%5B%7B%22key%22%3A%22workers_scripts%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22workers_kv_storage%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22d1%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_settings%22%2C%22type%22%3A%22read%22%7D%2C%7B%22key%22%3A%22workers_subdomain%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_analytics%22%2C%22type%22%3A%22read%22%7D%5D&accountId=*&zoneId=all&name=SRRoot-Deployer-Token" target="_blank" class="flex items-center justify-center w-full py-3.5 border border-orange-700 text-orange-500 bg-orange-900/20 hover:bg-orange-900/40 font-bold rounded-xl text-sm transition duration-300 shadow-sm">
                دریافت توکن کلودفلر
            </a>
<div class="mt-2 text-center mb-4">
    <p class="text-[11px] text-gray-500 dark:text-zinc-400 font-medium">
        ...
        کلیک کنید و توکن بسازید و آن را در کادر زیر وارد کنید.
            ...
            <div class="relative">
                <input type="password" id="apiToken" placeholder="توکن خود را وارد کنید" autocomplete="off" spellcheck="false" class="w-full pl-12 pr-4 py-3.5 bg-gray-50 dark:bg-sr-input border border-gray-300 dark:border-sr-border rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500 text-sm font-mono text-right text-gray-900 dark:text-zinc-100 transition token-input" dir="auto">
            ...
            <button id="deployBtn" onclick="startDeploy()" class="w-full py-3.5 border border-cyan-700 text-cyan-500 bg-cyan-900/20 hover:bg-cyan-900/40 font-black rounded-xl text-lg transition duration-300 shadow-sm glow-cyan">
                ⚡ ساخت پنل
            </button>
            <button type="button" id="openUpdateModalBtn" onclick="toggleUpdateModal(true)" class="w-full py-3.5 border border-violet-700 text-violet-500 bg-violet-900/20 hover:bg-violet-900/40 font-black rounded-xl text-lg transition duration-300 shadow-sm">
            ...
            <div id="status-container" class="hidden mt-4 bg-gray-50 dark:bg-zinc-900/50 rounded-xl p-4 border border-gray-200 dark:border-zinc-800/80">
                <div class="flex justify-between items-center mb-2.5">
                    ...
                    <span id="status-pct" class="text-xs font-black text-cyan-600 dark:text-cyan-500">۰٪</span>
                </div>
                <div class="w-full bg-gray-200 dark:bg-zinc-800 rounded-full h-1.5 overflow-hidden">
                    <div id="progressBar" class="bg-gradient-to-r from-cyan-500 to-violet-500 h-1.5 rounded-full transition-all duration-300" style="width: 0%"></div>
                </div>
            </div>
...
<div class="flex flex-col gap-4 mt-6 z-10">
    <div class="flex items-center gap-4 justify-center">
        ...
        <a href="https://t.me/SRRoot_Panel" target="_blank" class="flex items-center gap-2 px-4 py-2 border border-sky-700 text-sky-500 bg-sky-900/20 hover:bg-sky-900/40 rounded-full shadow-sm hover:shadow-md transition text-sm font-bold group">
            <svg class="w-5 h-5 group-hover:scale-110 transition" viewBox="0 0 24 24" fill="currentColor">
...
<div id="custom-confirm-modal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div id="custom-confirm-card" class="w-full max-w-sm bg-white dark:bg-sr-card border border-gray-200 dark:border-sr-border rounded-3xl shadow-2xl overflow-hidden p-6 text-center transform transition-all scale-95 duration-300">
        ...
        <div class="flex gap-3">
            ...
            <button id="custom-confirm-ok" 
                    class="flex-1 py-3 border border-cyan-700 text-cyan-500 bg-cyan-900/20 hover:bg-cyan-900/40 font-bold rounded-xl text-sm transition duration-200 shadow-sm">
                تایید
            </button>
        </div>
    </div>
...
function showToast(message, type = 'success') {
    ...
    const colors = type === 'error' 
        ...
        : 'bg-cyan-50 dark:bg-cyan-900/40 border-cyan-200 dark:border-cyan-800 text-cyan-600 dark:text-cyan-400';
            ...
            if (result.panels.length === 0) {
                ...
                result.panels.forEach(panel => {
                    ...
                    panelDiv.innerHTML = '<div class="flex flex-col">' +
                        ...
                        '<span id="version-text-' + panel.name + '" class="text-[11px] text-cyan-500 font-medium mt-1 animate-pulse" dir="rtl">در حال بررسی...</span>' +
                    '</div>' + 
...
async function fetchPanelVersion(token, scriptName, latestVersion, devSub) {
    try {
        ...
        if (versionText && btnContainer) {
            ...
            if (isLatest) {
                buttonsHtml += '<button disabled class="flex-1 px-4 py-1.5 border border-cyan-700 text-cyan-500 bg-cyan-900/20 font-bold rounded-xl text-[11px] cursor-not-allowed shadow-sm">آپدیت شده ✓</button>';
            } else {
                buttonsHtml += '<button data-name="' + scriptName + '" onclick="updateSrrootPanel(this.dataset.name)" class="flex-1 px-4 py-1.5 border border-violet-700 text-violet-500 bg-violet-900/20 hover:bg-violet-900/40 font-bold rounded-xl text-[11px] transition shadow-sm">آپدیت پنل</button>';
            }
            if (devSub) {
                buttonsHtml += '<a href="' + panelUrl + '" target="_blank" class="flex-1 px-4 py-1.5 border border-cyan-700 text-cyan-500 bg-cyan-900/20 hover:bg-cyan-900/40 font-bold rounded-xl text-[11px] transition shadow-sm flex items-center justify-center">ورود به پنل</a>';
            } else {
            ...
            buttonsHtml += '<button data-name="' + scriptName + '" onclick="reloadSrrootPanel(this.dataset.name)" class="flex-1 px-5 py-1.5 border border-cyan-700 text-cyan-500 bg-cyan-900/20 hover:bg-cyan-900/40 font-bold rounded-xl text-[11px] transition shadow-sm whitespace-nowrap min-w-[110px]">ری استارت</button>';
            buttonsHtml += '</div>';
            buttonsHtml += '<div class="flex gap-2">';
            buttonsHtml += '<button data-name="' + scriptName + '" onclick="deleteSrrootPanel(this.dataset.name)" class="flex-1 px-5 py-1.5 border border-red-700 text-red-500 bg-red-900/20 hover:bg-red-900/40 font-bold rounded-xl text-[11px] transition shadow-sm whitespace-nowrap min-w-[110px]">حذف پنل</button>';
            buttonsHtml += '</div></div>';
...
async function updateSrrootPanel(scriptName) {
    const token = document.getElementById('updateApiToken').value.trim();
    ...
    try {
        ...
        const result = await response.json();
        if (result.success) { showToast('✅ پنل ' + scriptName + ' با موفقیت آپدیت شد!'); setTimeout(() => checkExistingPanels(), 2000); }
        else { throw new Error(result.error); }
    } catch (e) { showToast('خطا: ' + e.message, 'error'); }
}

async function deleteSrrootPanel(scriptName) {
    const token = document.getElementById('updateApiToken').value.trim();
    ...
    try {
        ...
        const result = await response.json();
        if (result.success) { showToast('✅ پنل با موفقیت حذف شد'); setTimeout(() => checkExistingPanels(), 2000); }
        else { throw new Error(result.error); }
    } catch (e) { showToast('خطا: ' + e.message, 'error'); }
}
...
async function resetPanelPassword(scriptName) {
    ...
    try {
        ...
        const result = await response.json();
        if (result.success) { showToast('✅ رمز عبور بازنشانی شد'); setTimeout(() => checkExistingPanels(), 2000); }
        else { throw new Error(result.error); }
    } catch (e) { showToast('خطا: ' + e.message, 'error'); }
}

async function reloadSrrootPanel(scriptName) {
    const token = document.getElementById('updateApiToken').value.trim();
    if (!(await customConfirm('آیا پنل مجدداً دیپلوی شود؟'))) return;
    showToast('در حال ریلود پنل ' + scriptName + '...');
    try {
        const response = await fetch('/api/do-update', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token, scriptName }) });
        const result = await response.json();
        if (result.success) { showToast('✅ پنل با موفقیت ریلود شد'); setTimeout(() => checkExistingPanels(), 2000); }
        else { throw new Error(result.error); }
    } catch (e) { showToast('خطا: ' + e.message, 'error'); }
}
        ...
        async function startDeploy() {
            ...
            try {
                ...
                statusText.innerText = 'در حال دریافت پنل SRRoot...';
                ...
                if (result.success) {
                    ...
                    successText.className = 'text-center mt-6 font-bold text-sm text-cyan-600 dark:text-cyan-400 mb-3';
                    ...
                    linkBox.className = 'flex flex-col items-center justify-center p-3 bg-cyan-50 dark:bg-cyan-900/20 border border-cyan-500/50 rounded-xl mb-3';
                    const linkDisplay = document.createElement('span');
                    linkDisplay.className = 'text-sm font-mono text-cyan-700 dark:text-cyan-400 mb-2 text-center break-all';
                    linkDisplay.innerText = result.url;
                    linkDisplay.dir = 'ltr';
                    const copyBtn = document.createElement('button');
                    copyBtn.className = 'px-6 py-1.5 border border-cyan-700 text-cyan-500 bg-cyan-900/20 hover:bg-cyan-900/40 font-bold rounded-lg text-sm transition duration-300 shadow-sm';
                    copyBtn.innerText = 'کپی لینک پنل';
                    copyBtn.onclick = () => {
                        navigator.clipboard.writeText(result.url);
                        copyBtn.innerText = 'کپی شد!';
                        setTimeout(() => { copyBtn.innerText = 'کپی لینک پنل'; }, 2000);
                    };
                    ...
                    successLink.className = 'block w-full py-3.5 border border-violet-700 text-violet-500 bg-violet-900/20 hover:bg-violet-900/40 text-center font-bold rounded-xl transition duration-300 shadow-sm';
                ...
                if (errorMsg.includes("databases per account") || errorMsg.includes("limit reached")) {
                    errorBox.innerHTML = '<div class="mb-2 font-bold">شما به سقف مجاز ساخت دیتابیس D1 رسیده‌اید.</div><div class="text-[11px] opacity-70 mb-3" dir="ltr">' + rawError + '</div>';
                } else if (errorMsg.includes("script limit") || errorMsg.includes("scripts per account")) {
                    errorBox.innerHTML = '<div class="mb-2 font-bold">شما به سقف مجاز ساخت ورکر رسیده‌اید.</div><div class="text-[11px] opacity-70 mb-3" dir="ltr">' + rawError + '</div>';
                } else {
...
<div id="update-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 opacity-0 pointer-events-none transition-opacity duration-200 ease-out">
    <div id="update-modal-card" class="w-full max-w-md bg-white dark:bg-sr-card border border-gray-200 dark:border-sr-border rounded-3xl shadow-2xl p-5 transform transition-all scale-95 opacity-0 duration-200 flex flex-col max-h-[95vh]">
        ...
        <div class="space-y-4 shrink-0">
            <a href="https://dash.cloudflare.com/profile/api-tokens?permissionGroupKeys=%5B%7B%22key%22%3A%22workers_scripts%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22workers_kv_storage%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22d1%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_settings%22%2C%22type%22%3A%22read%22%7D%2C%7B%22key%22%3A%22workers_subdomain%22%2C%22type%22%3A%22edit%22%7D%2C%7B%22key%22%3A%22account_analytics%22%2C%22type%22%3A%22read%22%7D%5D&accountId=*&zoneId=all&name=SRRoot-Deployer-Token" target="_blank" class="flex items-center justify-center w-full py-2.5 border border-orange-700 text-orange-500 bg-orange-900/20 hover:bg-orange-900/40 font-bold rounded-xl text-sm transition duration-300 shadow-sm">
                دریافت توکن کلودفلر
            </a>
            <input type="password" id="updateApiToken" placeholder="توکن خود را وارد کنید" autocomplete="off" spellcheck="false" class="w-full px-4 py-3 bg-gray-50 dark:bg-sr-input border border-gray-300 dark:border-sr-border rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500 text-sm font-mono text-right text-gray-900 dark:text-zinc-100 transition" dir="auto">
            <button id="checkPanelsBtn" onclick="checkExistingPanels()" class="w-full py-3 border border-violet-700 text-violet-500 bg-violet-900/20 hover:bg-violet-900/40 font-bold rounded-xl text-md transition duration-300 shadow-sm">
                بررسی پنل‌های موجود
            </button>
        </div>
